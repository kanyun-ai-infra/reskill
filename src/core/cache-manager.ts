...
export class CacheManager {
  ...
  async get(
    ...
    // 读取缓存的 commit 信息
    const commitFile = path.join(cachePath, '.skpm-commit');
    let commit = '';
  ...
  async cache(
    ...
    // 保存 commit 信息
    const fs = await import('node:fs');
    fs.writeFileSync(path.join(cachePath, '.skpm-commit'), commit);
  ...
  async copyTo(
    ...
    copyDir(cached.path, destPath, { exclude: ['.skpm-commit'] });
  }
