...
import { buildRepoUrl, isGitUrl, parseGitUrl, GitCloneError, getRepoNameFromUrl } from './git.js';
  ...
  describe('GitCloneError', () => {
    ...
    describe('isAuthenticationError', () => {
      ...
      it('should detect could not read from remote', () => {
        expect(GitCloneError.isAuthenticationError('fatal: Could not read from remote repository')).toBe(true);
      });

      it('should detect authentication failed', () => {
        expect(GitCloneError.isAuthenticationError('Authentication failed for https://github.com')).toBe(true);
      });

      it('should detect repository not found', () => {
        expect(GitCloneError.isAuthenticationError('fatal: repository https://github.com/user/repo not found')).toBe(true);
      });
